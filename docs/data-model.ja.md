...
| `BUNDLE_SPEC_VERSION` | error | `bundle.spec_version != "2.1"`（STIX 2.1 仕様では bundle envelope の `spec_version` は廃止だが、SAGE が依存しているため TRACE は意図的に付与する。OASIS バリデータが返す `{401}` warning は受容） |
...
### TRACE 固有の bundle envelope（L4 メタデータ）

`build_stix_bundle_from_extraction` が **bundle ルート**に付与する。SAGE は
未知の `x_*` を無視するため後方互換性を維持。
