...
| `{202}` | `attack-pattern` source の `uses` relationship が `attack-pattern` を target にする | MITRE ATT&CK の sub-technique chaining（例: T1234 uses T1234.001）や技法合成を表現する canonical な方式。validator の suggested-target 表には未掲載だが、ATT&CK 整合 bundle では広く出現し、主要 STIX consumer は認識する。drop すると technique 階層が attack graph から欠落する。TRACE 1.0.1 で `tool uses` 系と同じ SHOULD-level 受容ロジックで accept。 |
...
#### identity_assets schema 進化 (Initiative C Phase 2 / TRACE 1.6.0)

`IdentityEntry` (`identity_assets.json[*].identities[]` の row 形状で BEACON
`Identity` model のミラー) は TRACE 1.6.0 で 2 つの optional field を追加。
BEACON 0.13.0 の emit 側拡張と対応:

| Field | Type | Default | 用途 |
|-------|------|---------|------|
| `is_high_value_impersonation_target` | `bool` | `false` | `true` の場合、TRACE の PIR L2 relevance gate が LLM verdict score に `+0.2` boost (1.0 cap) を加算。条件: 文書中に当該 identity 名 (case-insensitive substring) が出現。SAGE 0.9.0+ は flag を `effective_priority` 計算式で消費。 |
...
`IdentityEntry` の Pydantic model (`_StrictModel`、`extra="forbid"`) に明示
宣言が必要。宣言が無いと BEACON 0.13.0+ の `identity_assets.json` が validation で reject される。default で BEACON 0.12.x artifact との後方互換性確保。
