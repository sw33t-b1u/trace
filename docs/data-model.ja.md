...
| `prioritized_actors` | list[`ActorTriageEntry`] | はい | BEACON アクタートリアージ結果。各エントリは `actor_name`, `likelihood_score`, `confidence`, `intent`, `opportunity`, `capability` を持つ |
...
一部の OASIS 警告は spec の SHOULD レベルであり、TRACE は **意図的に
accept** する（捨てる代わりに脅威情報の semantic を保持する判断）。
`validate_stix --strict` で error に格上げ可能。

| コード | パターン | 受容理由 |
|-------|---------|---------|
| `{202}` | `tool` source の `uses` relationship が `malware` または `tool` を target にする | STIX 2.1 §4.13 は `(source_type, relationship_type)` 組み合わせごとに suggested target を定義する。`tool uses malware` / `tool uses tool` は suggested 集合に含まれないが semantic は明確（「攻撃ツール A が malware/tool B を利用する」）であり、incident report では頻出。MISP / OpenCTI 等の主要 consumer はこれらを問題なく ingest する。relationship を drop すると attack-graph の edge が失われる。 |
| `{202}` | `attack-pattern` source の `uses` relationship が `attack-pattern` を target にする | MITRE ATT&CK の sub-technique chaining（例: T1234 uses T1234.001）や技法合成を表現する canonical な方式。validator の suggested-target 表には未掲載だが、ATT&CK 整合 bundle では広く出現し、主要 STIX consumer は認識する。drop すると technique 階層が attack graph から欠落する。`tool uses` 系と同じ SHOULD-level 受容ロジックで accept。 |
...
#### extension-definition が宣言する型

extension は `extension_types: ["toplevel-property-extension", "new-sdo"]` を宣言する。`new-sdo` エントリは `x-asset-internal` および `x-identity-internal` を STIX 2.1 §7.3 準拠の new-SDO 型として正式登録する。両型は §7.3 準拠の `extensions` map エントリ `{"extension-definition--c1e4d6a7-…": {"extension_type": "new-sdo"}}` を持つ。

#### identity_assets なりすましフィールド

`IdentityEntry` (`identity_assets.json[*].identities[]` の row 形状で BEACON
`Identity` model のミラー) は 2 つの optional なりすましフィールドを持つ:

| Field | Type | Default | 用途 |
|-------|------|---------|------|
| `is_high_value_impersonation_target` | `bool` | `false` | `true` の場合、TRACE の PIR L2 relevance gate が LLM verdict score に `+0.2` boost (1.0 cap) を加算。条件: 文書中に当該 identity 名 (case-insensitive substring) が出現。SAGE は flag を `effective_priority` 計算式で消費。 |
| `impersonation_risk_factors` | `list[str]` | `[]` | 自由形式タグ (`["public-facing-brand", "executive", "trusted-supplier"]` が canonical example)。SAGE にそのまま流し、analyst 向け dashboard で使用。TRACE 自身は consume しない。 |

両フィールドは `IdentityEntry` の strict Pydantic model (`_StrictModel`、
`extra="forbid"`) に明示宣言されている。宣言が無いと BEACON の
`identity_assets.json` が `_StrictModel` の `extra="forbid"` で reject される。
両フィールドとも既定値は `false` / `[]`。

CLI 利用: `cmd/crawl_single.py --identity-assets PATH` で file 読み込み、
flag 付き identity 名を抽出して
`pir_relevance.evaluate(..., high_value_identity_names=...)` に渡す。
...
- 安定した自然キー（ATT&CK technique ID、正規化名、CVE ID）を持つ
  エンティティには決定的 UUIDv5 id を採番し、繰り返しクロールで同一
  STIX id に収束させる。自然キーのないエンティティ（indicator、identity、
  campaign 等）にはランダム UUIDv4 にフォールバック。relationship は
  常に UUIDv4
...
これにより、2 種のエラー（UUIDv4 違反 / timestamp 形式違反）が **構造的に発生しない**。残るのは語彙ミスマッチ
