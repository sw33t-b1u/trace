...
| `google-genai` | `>=1.0` | Vertex AI Gemini クライアント。L2 PIR リレバンスゲート（`gemini-2.5-flash`）と L3 STIX 抽出（`gemini-2.5-flash` / `pro`）で使用。BEACON と同じ SDK のため、複製した `llm/client.py` も SDK 境界で揃う。 | Apache-2.0 |
