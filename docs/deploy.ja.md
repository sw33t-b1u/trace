...
TRACE は、PIR ベースのスケジュール収集用 `trace-crawl` Cloud Run Job として単独運用できる。
ブラウザ完結の CTI workflow では、BEACON リポジトリから `cti-console` をデプロイする。
その image は既定で最新の TRACE main を使用し、`TRACE_ROOT_PATH=/app/trace` を設定するため、
Collection タブは `trace discover-pir` と `trace crawl-batch` をローカルに実行できる。
console の build/deploy command と、再現可能な build 用の任意 `TRACE_REF` pin は BEACON deployment guide を参照する。
