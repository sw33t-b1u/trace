...
その image は互換性のある TRACE ref を pin し、`TRACE_ROOT_PATH=/app/trace` を設定するため、
Collection タブは `trace discover-pir` と `trace crawl-batch` をローカルに実行できる。
console の build/deploy command と BEACON↔TRACE 互換 pin は BEACON deployment guide を参照する。
