...
| `google-genai` | `>=1.0` | Vertex AI Gemini client for the L2 PIR relevance gate (`gemini-2.5-flash`) and L3 STIX extraction (`gemini-2.5-flash` / `pro`). Same SDK BEACON uses, so the duplicated `llm/client.py` stays in sync at the SDK boundary. | Apache-2.0 |
