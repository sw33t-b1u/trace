...
│                       #   主要 docs: setup.md, deploy.md, usage.md,
│                       #   data-model.md, crawl_design.md, structure.md,
│                       #   dependencies.md, api-stability.md。
│                       #   high-level-design.md を含む
│                       #   (maintainer 方針で gitignored;
│                       #   `.gitignore` エントリ `docs/high-level-design.md`
│                       #   参照)。
...
| `llm/` | Vertex AI Gemini クライアント + プロンプトアセット。Decision I-12 に従い BEACON から複製（CHANGELOG 0.1.0 参照）。 |
| `pir/` | L2 PIR 関連性ゲート。 |
| `review/` | 人手レビューハンドオフ (Markdown / GHE Issue)。 |
| `stix/` | L3 抽出、L4 バンドル組み立て、taxonomy エンリッチ、外部参照ハッシュ。 |
| `storage/` | StorageBackend 抽象化 (ローカル / GCS)。ファクトリ、ABC、2 実装。 |
