...
TRACE は統合 `trace` コンソールスクリプトで 14 個のサブコマンドを提供する:
...
| `trace discover-pir` | RSS/Atom ソースカタログから PIR に合致する記事候補を探索する |
...
> TRACE は `uv run trace <subcommand>` で呼び出す。
...
| フラグ | 対象コマンド | 説明 |
|-------|------------|------|
| `--pir <path>` | `crawl-single`, `crawl-batch`, `discover-pir` | `pir_output.json` のパス。crawl 系では L2/L3/L4 を有効化し、`discover-pir` では探索コンテキストになる |
...
| `--catalog <path>` | `discover-pir` | RSS/Atom 記事探索に使うソースカタログ YAML |
| `--from`, `--to`, `--since-days` | `discover-pir` | 探索対象の期間 |
| `--max-candidates <N>` | `discover-pir` | 候補 JSON の最大件数 |
| `--include-recent` | `discover-pir` | PIR term に合致しない期間内の recent 記事も含める |
| `--open-issue` | `submit-review` | 検証レポートを GitHub Issue として投稿する |
| `--json` | `search-iocs`, `discover-pir` | 結果を人間可読テキストではなく JSON で出力する |
...
- **L2 — リレバンスゲート**: 軽量な `gemini-3.1-flash-lite` 呼び出しが記事を PIR
...
`trace discover-pir` は BEACON PIR の term と合致する記事候補を
RSS/Atom ソースカタログから探索する。これは STIX 抽出を**行わない**。出力は
BEACON Collection UI または運用者レビューで人間が承認するための候補 JSON
である。承認済み URL は `trace crawl-batch --pir` に渡され、通常の L2/L3/L4
パイプラインで処理される。

```bash
# input/source_catalog.yaml から候補探索する。運用者カタログがない場合は
# コミット済み example catalog にフォールバックする。
...
コミット済みの `input/source_catalog.example.yaml` はテンプレートである。運用者は
これを gitignored の `input/source_catalog.yaml` にコピーし、自環境の feed 一覧に
合わせて調整する。カタログ、候補 JSON、source ごとの診断、`--include-recent` fallback については
[`docs/crawl_design.md`](crawl_design.md) を参照。
...
| `TRACE_STORAGE_BUCKET` | — | GCS バケット名（`TRACE_STORAGE=gcs` 時に必須）|
| `TRACE_STORAGE_PREFIX` | (空文字) | GCS バケット内のキープレフィックス |
